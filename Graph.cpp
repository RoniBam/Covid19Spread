...
Tree* Graph ::BFS(int rootLabel, Session& session) {
    ...
    Tree* toReturn= buildBFSRec(session,rootLabel,visited);
    delete notnull;
    return toReturn;
}

Tree *Graph::buildBFSRec(Session &session, int index, std::vector<Tree*> &visited) {
    ...
    for (unsigned int i = 0; i < edges.size(); i++) {
        if (visited[i] == curr) {
            Tree* temp=buildBFSRec(session, i, visited);
            curr->addChild(*temp);
            delete temp;
        }
    }
